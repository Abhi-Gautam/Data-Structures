...
void dfs(VI g[], int u, vector<bool> &visit)
{
    visit[u] = true;
    for(int i = 0; i < g[u].size(); i++)
    {
        if(!visit[g[u][i]]) dfs(g, g[u][i], visit);
    }
}


bool isConnected(VI g[], vector<bool> &mark, int s)
{
    ...
    for(int i = 0; i < M; i++)
    {
        if(mark[i] && !visit[i]) return false;
    }

    return true;
}

bool possibleOrderAmongStrings(string s[], int n)
{
    VI g[M];
...
int main()
{
    ...
    if(possibleOrderAmongStrings(s, n) == false) cout<<"Not Possible"<<endl;
