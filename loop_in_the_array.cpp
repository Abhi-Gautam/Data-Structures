...
bool isCycle(int v, VI adj[], vector<bool> visited, vector<bool> recur)
{
    visited[v] = true;
    recur[v] = true;
    rep(i, adj[v].size())
    {
        ...
        else if(visited[adj[v][i]] == true && recur[adj[v][i]] == true) return true;
    }
...
int main()
{
    ...
    rep(i, n)
    {
        if(i != (i+v[i]+n)%n) adj[i].PB((i+v[i]+n)%n);
    }
