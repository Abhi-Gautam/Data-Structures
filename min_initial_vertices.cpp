...
void dfs(int n, int m, bool vis[][MAX], int adj[][MAX], int N, int M)
{
    vis[n][m] = 1;
    if(n+1 < N && adj[n][m] >= adj[n+1][m] && !vis[n+1][m]) dfs(n+1, m, vis, adj, N, M);
    if(m+1 < M && adj[n][m] >= adj[n][m+1] && !vis[n][m+1]) dfs(n, m+1, vis, adj, N, M);
    if(n-1 >= 0 && adj[n][m] >= adj[n-1][m] && !vis[n-1][m]) dfs(n-1, m, vis, adj, N, M);
    if(m-1 >= 0 && adj[n][m] >= adj[n][m-1] && !vis[n][m-1]) dfs(n, m-1, vis, adj, N, M);
}

void printMinSources(int adj[][MAX], int N, int M)
{
    vector<pair<long int, pair<int , int>>> x;
    rep(i, N) rep(j, M) x.PB(MP(adj[i][j],MP(i, j)));
    sort(x.begin(), x.end());
    ...
    rep(i, x.size())
    {
        if(!vis[x[i].second.first][x[i].second.second])
        {
            ...
            dfs(x[i].second.first, x[i].second.second, vis, adj, N, M);
        }
    }
}
