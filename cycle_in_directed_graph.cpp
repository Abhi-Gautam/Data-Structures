...
bool dfs(int i, bool *vis, bool *recurStack, VI adj[])
{
    if(vis[i] == false)
    {
        vis[i] = true;
        ...
        {
            if(!vis[i] && dfs(i, vis, recurStack, adj)) return true;
            else if (recurStack[i]) return true;
        }
    }
...
bool isCyclic(int V, VI adj[])
{
    ...
    rep(i, V) visited[i] = false, recurStack[i] = false;
