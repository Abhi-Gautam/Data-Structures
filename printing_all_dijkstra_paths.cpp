...
void shortestPath(VII adj[], int V, int src)
{
...
void printPath(VI parent, int j)
{
    if (parent[j] == -1)
        return;
...
void printAllSolutions(VI parent, int V)
{
    for(int i = 1; i < V; ++i)
    {
        printPath(parent, i);
...
int main()
{
    ...
    parent[0] = -1;
    shortestPath(adj, nodes, 0);
    printAllSolutions(parent, nodes);
    // rep(i, nodes) cout<<parent[i]<<" ";
    return 0;
}