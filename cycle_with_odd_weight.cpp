...
bool twoColorUtil(VI adj[], int src, int n, int color[])
{
    color[src] = 1;
...
bool twoColors(VI adj[], int n)
{
    ...
    for(int i = 1; i <= n; ++i)
    {
        if(color[i] == -1)
        {
            if(twoColorUtil(adj, i, n, color) == false)
                return false;
        }
    }
    return true;
...
bool isOddSum(int info[][3], int n, int m)
{
    ...
    return twoColors(adj, n+pseudo_count);
}
...
int main()
{
    ...
    if(isOddSum(info, n, m) == true) cout<<"No"<<endl;
    else cout<<"Yes"<<endl;
    return 0;
}